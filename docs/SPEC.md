...
**What it is:** A local CLI that records your laptop's system audio during a call and drops a timestamped markdown transcript on disk. A coding agent then refines it into clean notes on disk (see AGENTS.md).
...
- `watch.py`: Linux-side watcher, blocks until a new transcript lands
- `AGENTS.md`: procedure any coding agent follows to refine and summarise (CLAUDE.md imports it)

**Input:** Whatever is playing through your default speakers.
**Output:** `notes/raw/YYYY-MM-DD_HHMMSS.wav` (16kHz mono WAV) and `notes/transcripts/YYYY-MM-DD_HHMMSS.md` (timestamped transcript), same timestamp, both kept on disk. `notes/refined/<stem>.md` and `notes/summaries/<stem>.md`, agent-written, same stem.
**Interface:** Run `./start.sh`, join your call, press Enter when done to stop recording and start transcription. Flags: `--seconds N`, `--model NAME`, `--wav PATH`, `--no-transcribe`. Then ask a coding agent to run the transcription program; it waits with `watch.py` and writes the refined transcript and notes.
...
**Sat Sep 5, evening: DONE**
Agent harness: `watch.py`, `AGENTS.md`, `CLAUDE.md`, atomic transcript write, ETA line.
...
**Next (not scheduled):** `--mic` capture of your own side of a call; `small.en` default for calls over 30 minutes; incremental WAV writing for multi-hour recordings; live chunked transcription.
